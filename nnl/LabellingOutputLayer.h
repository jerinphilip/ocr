...
#ifndef _INCLUDED_LabellingOutputLayer_h  
#define _INCLUDED_LabellingOutputLayer_h  
...
private:
	...
	vector<double> logOneMinusActBuffer;
	vector<string> labels;
	string name;
	vector<vector<double> > forwardVars;
	vector<vector<double> > backwardVars;
	vector<int> seqDims;
	
	//functions
	void getMostProbableLabelCounts(vector<int>& labelCounts);

public:

	LabellingOutputLayer(const string& nam, const vector<string>& lab, int numDims, vector<string>& criteria);
	~LabellingOutputLayer();
